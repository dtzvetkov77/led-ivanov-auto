...
export default function AdminPartnersPage() {
  ...
  async function handleDelete(id: string, name: string) {
    if (!confirm(`Изтриване на "${name}"? Действието е необратимо.`)) return
    await supabase.from('partner_images').delete().eq('partner_id', id)
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 text-xs text-red-400 hover:text-red-300 border border-border hover:border-red-500/50 px-3 py-1.5 rounded-lg transition-colors"
                      >
                        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
