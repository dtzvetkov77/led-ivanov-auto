...
export default async function HomePage() {
  ...
  const [{ data: products }, { data: partnersData }, { data: categoriesData }] = await Promise.all([
    ...
    supabase.from('partners').select('slug,name,city,phone,cover_image,logo_url').eq('published', true).order('position'),
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {[
            ...
            },
            {
              ...
              title: 'до 2 год. гаранция', sub: 'На всички продукти',
            },
            {
    ...
    <section className="py-16 px-4 bg-surface border-y border-border">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <h2 className="text-2xl md:text-3xl font-black mb-2">НАШИТЕ УСЛУГИ</h2>
          <p className="text-muted text-sm">Професионална грижа за фаровете на вашия автомобил</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {previews.map(s => (
            ...
            >
              ...
              {/* Content */}
              <div className="p-5">
                <h3 className="font-black text-base mb-2 group-hover:text-accent transition-colors">{s.name}</h3>
